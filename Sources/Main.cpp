...
void stepping(double x)
{
    for (double h = 1e-13; h < 1; h *= 1.1)
    {
        auto[val, it] = task8({0.3802337646484376, 0.380291748046875}, Functions::f, h);
...
int main()
{
    ...
    double n = variant + 3;
    double A[] = {
            (-7 + 15 * n) / 2, (1 + n) / 2, (7 - n) / 2, (1 + n) / 2, (11 - 13 * n) / 2, (-7 + 9 * n) / 2, 1 + n,
            2 * (-1 + n), (3 - 5 * n) / 2, (-5 + 11 * n) / 2, (1 - 3 * n) / 2, (1 + n) / 2, (-1 + 3 * n) / 2,
            (1 - 3 * n) / 2, (-1 + 3 * n) / 2, (1 - 3 * n) / 2, 0, 0, (-1 + 3 * n) / 2, (1 - 3 * n) / 2,
            -29.0 / 2 + 2 * n, -5.0 / 2 + 2 * n, 5.0 / 2 + n, -5.0 / 2 + 2 * n, 29.0 / 2 - 2 * n, -21.0 / 2 + 2 * n, 0,
            -4, 13.0 / 2 - 2 * n, -21.0 / 2 + 2 * n, (17 + 3 * n) / 2, (5 + n) / 2, (-5 + n) / 2, 5 * (1 + n) / 2,
            -3 * (7 + n) / 2, 3 * (5 + n) / 2, -2, 5, -3 * (5 + n) / 2, 3 * (7 + n) / 2, (-55 + 9 * n) / 2,
            (-11 + n) / 2, (11 - n) / 2, (-11 + n) / 2, (63 - 11 * n) / 2, 9 * (-5 + n) / 2,
            3 - n, -11 + 2 * n, (33 - 5 * n) / 2, 7 * (-7 + n) / 2, -59.0 / 2 + 5 * n, -11.0 / 2 + n, 11.0 / 2 - n,
            -11.0 / 2 + n, 63.0 / 2 - 7 * n, -43.0 / 2 + 6 * n, 2 - 2 * n, 2 * (-6 + n), 35.0 / 2 - 3 * n,
            -55.0 / 2 + 3 * n, -2 + 5 * n / 2, n / 2, -n / 2, n / 2, 3 - 7 * n / 2, -2 + 5 * n / 2, -n, -1 + n,
            1 - 3 * n / 2, -1 + 3 * n / 2, -5.0 / 2 - 4 * n, -5.0 / 2, -3.0 / 2, -5.0 / 2, 11.0 / 2 + 3 * n,
            -11.0 / 2 - 2 * n, 1 - n, 0, 5.0 / 2 + n, -1.0 / 2 - 2 * n, 5 * (5 + 2 * n), 7 + 2 * n, -3 - 2 * n,
            7 + 2 * n, -30 - 11 * n, 23 + 8 * n, -3 - n, 3 * (3 + n), -4 * (4 + n), 21 + 6 * n, 8 + 3 * n, 2 + n,
            -2 - n, 2 + n, -11 - 4 * n, 8 + 3 * n, -1 - n, 3 + n, -5 - 2 * n, 2 * (3 + n)};
    //loop();
    stepping(1);
